(function () {
    ...
    app.controller("mapController", ["$http", "$uibModalInstance", "NgMap", "tripInfo", "utils", function ($http, $uibModalInstance, NgMap, tripInfo, utils) {
        ...
        $http({
            ...
            ctrl.origin = {
                location: {
                    lat: ctrl.markers[0].Latitude,
                    lng: ctrl.markers[0].Longtitude
                }
            };
            ctrl.destination = {
                location: {
                    lat: ctrl.markers[ctrl.markers.length - 1].Latitude,
                    lng: ctrl.markers[ctrl.markers.length - 1].Longtitude
                }
            };
            for (var i = 1; i < ctrl.markers.length - 1; i++) {
                ctrl.waypoints.push({
                    location: {
                        lat: ctrl.markers[i].Latitude,
                        lng: ctrl.markers[i].Longtitude
                    }
                });
            }
        });
